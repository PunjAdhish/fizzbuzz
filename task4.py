...
r_resp1=response.json()
deviceId[]=r_resp1['response'][0]['deviceId']
macAddess[]=r_resp1p['response'][0]['macAddess']
result_dict=[]
key_tup=()
for j  in range (0,len(deviceId)):
	key_tup=key_tup+deviceId[j]
	key_tup=key_tup+macaddress[j]
	result_dict=result_dict+{key_tup:portName[j]}
	key_tup=()
	inc+=1
print(result_dict)
def gethostcount(inc):
	print("Total host"+inc)
